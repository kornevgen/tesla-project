...
public class TlbMissL1Hit implements DomainConstraint
{
	...
	public TlbMissL1Hit(List<Tagset> previousTagsets, TLB tlb, Cache l1)
	{
		...
		constants = new Domain( l1.getTagsets() ).getIntersectWith(
				new Domain( tlb.getNotMicroPfns(), new HashSet<Long>() )
			).tagsetConstants;
		
		//allowed = {x +> [PFN\M] \inter values(x[..])}
		// allowed ==> (ts1) ts = ts1 /\ ts[...] \in allowed(ts1) << [PFN\M]
		...
		for( Tagset ts : evicting )
		{
			...
			Domain dtsInterM = dts.getIntersectWith( new Domain( tlb.getNotMicroPfns(), new HashSet<Long>() ) );
		...
		Domain micro = new Domain( tlb.getNotMicroPfns(), new HashSet<Long>() );
		// [PFN\M] \inter L1
