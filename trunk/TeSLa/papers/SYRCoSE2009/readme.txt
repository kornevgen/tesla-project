...
#############################
новый вариант доклада (27.02.2009)
1. поскольку немногие из присутствующих тут даже просто сталкиваются с тестированием микропроцессоров, позволю себе пояснить, что это такое.
Однако многим знакомы unit-тесты: система запускается, инициализируется, над нею выполняются операции и в конце анализируется результат.
Всё это в легкостью переносится и на микропроцессоры: в качестве unit-теста выступает ассемблерная программа, которую можно запустить на процессоре. В ней может быть зафиксированы ожидаемые значения регистров и, если они не таковы, программа может об этом сигнализировать.
Получается этакое тестирование микропроцессора методом тестирования черного ящика. Или, по-другому, системное функциональное тестирование.
Однако кроме регистров микропроцессор управляет другими своими подсистемами, например, кэш-памятью.
Но ведь кэш-память не проверишь из ассемблерной программы - кэш работает скрыто от программиста.
Но всё же можно по ходу работы микропроцессора отлавливать то, что происходит в кэш-памяти: а именно, кэш-промахи и кэш-попадания.

Главный вопрос - откуда взять тестовые программы ?
Первое, что приходит на ум, случайная генерация.
Но чтобы проводить тестирование целенаправленно, был разработан метод нацеленной генерации.
Попросту говоря, надо перебирать разные инст