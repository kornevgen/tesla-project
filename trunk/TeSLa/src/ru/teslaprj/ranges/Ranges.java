...
//TODO без этого по€вл€ютс€ ложные consistent!	yl.yicesl_read(context, "(check)");
			...
			yl.yicesl_read(context, "(check)");// без этого не по€вл€етс€ модель...
...
//			addition axiom: if type(ts1)=type(ts2) and (ts1:0 \/ ts1:1) -> (value_ts(ts1) = value_ts(ts2) <=> ts1 = ts2)
			...
			// лишн€€ работа, но это способ избавитьс€ от кванторов; видимо, из-за них ошибка
